...
RankLyft runs seven cron jobs configured in `vercel.ts`. All jobs run in the Vercel Production environment only. They do not run on Preview or Development deployments.
...
### Sync GA4 — daily at 8:00 AM UTC

Imports Google Analytics 4 traffic data for all users with a connected Google account. Fetches the last 3 days of daily rows and the last 30 days of page/country aggregated data.

See [Cron: Sync GA4](/reference/cron-sync-ga4) for full details.
